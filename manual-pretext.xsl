...
<p>
	<code>pandoc</code> [<em>options</em>] [<em>input-file</em>]…
	</p>

<h1 id="description">Description</h1>

<p>
	Pandoc is a <a href='https://www.haskell.org' title=''>Haskell</a> library for converting from one markup format to
...
Pandoc can also produce <a href='https://www.adobe.com/pdf/' title=''>PDF</a> output: see <a href='#creating-a-pdf' title=''>creating a PDF</a>, below.
	</p>

<p>
	Pandoc’s enhanced version of Markdown includes syntax for <a href='#tables' title=''>tables</a>,
...
and much more. See below under <a href='#pandocs-markdown' title=''>Pandoc’s Markdown</a>.
	</p>
...
run custom <a href='http://pandoc.org/filters.html' title=''>pandoc filters</a> to modify the intermediate AST.
	</p>
...
use the <code>-s</code> or <code>--standalone</code> flag:
	</p>
...
<a href='#templates' title=''>Templates</a> below.
	</p>

<p>
	If multiple input files are given, <code>pandoc</code> will concatenate them all (with
...
Thus, to convert <code>hello.txt</code> from Markdown to LaTeX, you could type:
	</p>
...
<p>
	Supported input and output formats are listed below under <a href='#options' title=''>Options</a>
...
<p>
	If the input or output format is not specified explicitly, <code>pandoc</code>
...
<p>
	will convert <code>hello.txt</code> from Markdown to LaTeX. If no output file
...
should pipe input and output through <a href='http://www.gnu.org/software/libiconv/' title=''><code>iconv</code></a>:
	</p>
...
will only be included if you use the <code>-s/--standalone</code> option.
	</p>

<h2 id="creating-a-pdf">Creating a PDF</h2>

<p>
	To produce a PDF, specify an output file with a <code>.pdf</code> extension:
	</p>
...
<p>
	Alternatively, pandoc can use <a href='http://www.contextgarden.net/' title=''>ConTeXt</a>, <code>pdfroff</code>, or any of the
...
to the command line (<code>-t html</code> defaults to <code>--pdf-engine=wkhtmltopdf</code>).
	</p>

<p>
	PDF output uses <a href='#variables-for-latex' title=''>variables for LaTeX</a> (with a LaTeX engine);
...
(an HTML/CSS-to-PDF engine; <code>--css</code> also affects the output).
	</p>

<p>
	To debug the PDF creation, it can be useful to look at the intermediate
representation: instead of <code>-o test.pdf</code>, use for example <code>-s -o test.tex</code>
to output the generated LaTeX. You can then test it with <code>pdflatex test.tex</code>.
	</p>
...
and <a href='https://ctan.org/pkg/footnotehyper' title=''><code>footnotehyper</code></a> or <a href='https://ctan.org/pkg/footnote' title=''><code>footnote</code></a> (to allow footnotes in tables).
	</p>
...
<p>
	Note that <code>odt</code>, <code>docx</code>, and <code>epub</code> output will not be directed
to <em>stdout</em> unless forced with <code>-o -</code>.
	</p>
...
names. See <code>--list-output-formats</code> and <code>--list-extensions</code>, below.
	</p></dd>
...
(<code>docx</code>, <code>odt</code>, <code>epub2</code>, <code>epub3</code>) is specified.
	</p></dd>
...
with pandoc, add this to your <code>.bashrc</code>:
	</p>
...
regardless of verbosity settings (<code>--verbose</code>, <code>--quiet</code>).
	</p></dd>
...
<dd><p>
	<em>Deprecated. Use the <code>+empty_paragraphs</code> extension instead.</em>
...
files (docx, odt, epub) implies <code>--file-scope</code>.
	</p></dd>
...
<p>
	Filters may be written in any language. <code>Text.Pandoc.JSON</code>
...
<a href='https://github.com/mvhenderson/pandoc-filter-node' title=''>JavaScript/node.js</a>.
	</p>
...
<li><p>
	<code>$DATADIR/filters</code> (executable or non-executable)
where <code>$DATADIR</code> is the user data directory (see
<code>--data-dir</code>, above).
	</p></li>
<li><p>
	<code>$PATH</code> (executable only)
	</p></li>
</ol>
...
<p>
	The <code>pandoc</code> lua module provides helper functions for element
...
produced by the MS Word <q>Track Changes</q> feature. <code>accept</code> (the
...
<dd><p>
	Print the system default template for an output <em>FORMAT</em>. (See <code>-t</code>
...
<code>--print-default-data-file</code> on the command line.
	</p></dd>
<dt><code>--eol=crlf</code>|<code>lf</code>|<code>native</code></dt>
<dd><p>
	Manually specify line endings: <code>crlf</code> (Windows), <code>lf</code>
...
calculation of column widths for plain text tables (see <a href='#tables' title=''>Tables</a> below).
	</p></dd>
...
on <code>man</code>, <code>docbook4</code>, <code>docbook5</code>, or <code>jats</code> output.
	</p></dd>
...
<code>--list-highlight-styles</code>.
	</p>
...
use <code>--print-highlight-style</code>.
	</p></dd>
...
order specified. Implies <code>--standalone</code>.
	</p></dd>
...
<p>
	<code>--resource-path</code> only has an effect if (a) the output
...
the environment variable <code>http_proxy</code> to <code>http://...</code>.
	</p></dd>
</dl>
...
in an offline <q>self-contained</q> <code>reveal.js</code> slide show.
	</p></dd>
...
<dd><p>
	Specify whether footnotes (and references, if <code>reference-links</code> is
set) are placed at the end of the current (top-level) block, the
current section, or the document. The default is
<code>document</code>. Currently only affects the markdown writer.
	</p></dd>
...
This option also affects markdown cells in <code>ipynb</code> output.
	</p></dd>
...
document to be numbered <q>6</q>, specify <code>--number-offset=5</code>.
If your document starts with a level-2 header which you want to
be numbered <q>1.5</q>, specify <code>--number-offset=1,4</code>.
Offsets are 0 by default. Implies <code>--number-sections</code>.
	</p></dd>
<dt><code>--listings</code></dt>
<dd><p>
	Use the <a href='https://ctan.org/pkg/listings' title=''><code>listings</code></a> package for LaTeX code blocks. The package
...
documented here: <a href='https://en.wikibooks.org/wiki/LaTeX/Source_Code_Listings#Encoding_issue' title=''>Encoding issue with the listings package</a>.
	</p></dd>
...
<dd><p>
	Wrap sections in <code>&lt;section&gt;</code> tags (or <code>&lt;div&gt;</code> tags for <code>html4</code>),
and attach identifiers to the enclosing <code>&lt;section&gt;</code> (or <code>&lt;div&gt;</code>)
rather than the header itself. See
<a href='#header-identifiers' title=''>Header identifiers</a>, below.
	</p></dd>
<dt><code>--email-obfuscation=none</code>|<code>javascript</code>|<code>references</code></dt>
<dd><p>
	Specify a method for obfuscating <code>mailto:</code> links in HTML documents.
...
<p>
	To produce a custom <code>reference.docx</code>, first get a copy of
...
<p>
	To produce a custom <code>reference.odt</code>, first get a copy of
...
<code>Home</code> menu to check.)
	</p>

<p>
	You can also modify the default <code>reference.pptx</code>: first run
...
in a YAML metadata block (see <a href='#epub-metadata' title=''>EPUB Metadata</a>, below).
	</p></dd>
...
<a href='#epub-metadata' title=''>EPUB Metadata</a>.
	</p></dd>
...
<dd><p>
	Determines how ipynb output cells are treated. <code>all</code> means
...
with the output format. The default is <code>best</code>.
	</p></dd>
...
<dd><p>
	Use the given string as a command-line argument to the <code>pdf-engine</code>.
...
<dd><p>
	Set the <code>bibliography</code> field in the document’s metadata to <em>FILE</em>,
...
<dd><p>
	Set the <code>csl</code> field in the document’s metadata to <em>FILE</em>,
overriding any value set in the metadata. (This is equivalent to
<code>--metadata csl=FILE</code>.)
This option is only relevant with <code>pandoc-citeproc</code>.
	</p></dd>
<dt><code>--citation-abbreviations=</code><em>FILE</em></dt>
<dd><p>
	Set the <code>citation-abbreviations</code> field in the document’s metadata to
...
<dd><p>
	Use <a href='https://ctan.org/pkg/natbib' title=''><code>natbib</code></a> for citations in LaTeX output. This option is not for use
with the <code>pandoc-citeproc</code> filter or with PDF output. It is intended for
use in producing a LaTeX file that can be processed with <a href='https://ctan.org/pkg/bibtex' title=''><code>bibtex</code></a>.
	</p></dd>
<dt><code>--biblatex</code></dt>
<dd><p>
	Use <a href='https://ctan.org/pkg/biblatex' title=''><code>biblatex</code></a> for citations in LaTeX output. This option is not for use
with the <code>pandoc-citeproc</code> filter or with PDF output. It is intended for
use in producing a LaTeX file that can be processed with <a href='https://ctan.org/pkg/bibtex' title=''><code>bibtex</code></a> or <a href='https://ctan.org/pkg/biber' title=''><code>biber</code></a>.
	</p></dd>
</dl>
...
<dd><p>
	Use <a href='https://www.mathjax.org' title=''>MathJax</a> to display embedded TeX math in HTML output.
...
<dd><p>
	Convert TeX math to <a href='http://www.w3.org/Math/' title=''>MathML</a> (in <code>epub3</code>, <code>docbook4</code>, <code>docbook5</code>, <code>jats</code>,
...
<dd><p>
	Convert TeX formulas to <code>&lt;img&gt;</code> tags that link to an external script
...
<dd><p>
	Use <a href='https://github.com/Khan/KaTeX' title=''>KaTeX</a> to display embedded TeX math in HTML output.
...
<dd><p>
	Enclose TeX math in <code>&lt;eq&gt;</code> tags in HTML output. The resulting HTML
...
after a <code>--</code> separator at the end of the line.
	</p></dd>
...
<p>
	When the <code>-s/--standalone</code> option is used, pandoc uses a template to
...
directory (see <code>--data-dir</code>, above). <em>Exceptions:</em>
	</p>
...
or with the <code>-M/--metadata</code> option.
	</p>
...
<p>
	will include <code>title</code>, <code>author</code> and <code>description</code> as standard document
...
when using LaTeX (through <a href='https://ctan.org/pkg/babel' title=''><code>babel</code></a> and <a href='https://ctan.org/pkg/polyglossia' title=''><code>polyglossia</code></a>) or ConTeXt.
	</p>

<p>
	Use native pandoc <a href='#divs-and-spans' title=''>Divs and Spans</a> with the <code>lang</code> attribute to
switch the language:
	</p>
...
<p>
	For bidirectional documents, native pandoc <code>span</code>s and <code>div</code>s
...
<a href='http://www.w3.org/International/articles/inline-bidi-markup/uba-basics' title=''>Unicode Bidirectional Algorithm</a>.
	</p>

<p>
	When using LaTeX for bidirectional documents, only the <code>xelatex</code> engine
is fully supported (use <code>--pdf-engine=xelatex</code>).
	</p></dd>
</dl>
...
<p>
	These affect HTML output when <a href='#producing-slide-shows-with-pandoc' title=''>producing slide shows with pandoc</a>.
All <a href='https://github.com/hakimel/reveal.js#configuration' title=''>reveal.js configuration options</a> are available as variables.
	</p>
...
<p>
	These variables change the appearance of PDF slides using <a href='https://ctan.org/pkg/beamer' title=''><code>beamer</code></a>.
	</p>
...
<p>
	Pandoc uses these variables when <a href='#creating-a-pdf' title=''>creating a PDF</a> with a LaTeX engine.
	</p>
...
(old-style) figures through the <a href='https://ctan.org/pkg/libertinus' title=''><code>libertinus</code></a> package:
	</p>
...
to use the <a href='http://www.gust.org.pl/projects/e-foundry/tex-gyre' title=''>TeX Gyre</a> version of Palatino with lowercase figures:
	</p>
...
<p>
	These variables function when using BibLaTeX for <a href='#citation-rendering' title=''>citation rendering</a>.
	</p>
...
<p>
	Pandoc uses these variables when <a href='#creating-a-pdf' title=''>creating a PDF</a> with ConTeXt.
	</p>
...
<p>
	Pandoc uses these variables when <a href='#creating-a-pdf' title=''>creating a PDF</a> with <a href='https://wkhtmltopdf.org' title=''><code>wkhtmltopdf</code></a>.
The <code>--css</code> option also affects the output.
	</p>
...
<p>
	Pandoc sets these variables automatically in response to <a href='#options' title=''>options</a> or
...
<p>
	Variable names are sequences of alphanumerics, <code>-</code>, and <code>_</code>,
starting with a letter. A variable name surrounded by <code>$</code> signs
will be replaced by its value. For example, the string <code>$title$</code> in
	</p>
...
<p>
	This will include <code>X</code> in the template if <code>variable</code> has a truthy
...
<p>
	<code>X</code> and <code>Y</code> are placeholders for any valid template text,
and may include interpolated variables or other conditionals.
The <code>$else$</code> section may be omitted.
	</p>

<p>
	When variables can have multiple values (for example, <code>author</code> in
a multi-author document), you can use the <code>$for$</code> keyword:
	</p>
...
<p>
	Note that the separator needs to be specified immediately before the <code>$endfor</code>
keyword.
	</p>
...
<p>
	Templates may contain comments: anything on a line after <code>$--</code>
...
<p>
	An extension can be enabled by adding <code>+EXTENSION</code>
...
<p>
	Note that markdown extensions added to the <code>ipynb</code> format
affect Markdown cells in Jupyter notebooks (as do command-line
options like <code>--atx-headers</code>).
	</p>
...
<p>
	Interpret straight quotes as curly quotes, <code>---</code> as em-dashes,
...
<p>
	Note: If you are <em>writing</em> Markdown, then the <code>smart</code> extension
has the reverse effect: what would have been curly quotes comes
out straight.
	</p>

<p>
	In LaTeX, <code>smart</code> means to use the standard TeX ligatures
...
third with <code>-2</code>; and so on.
	</p>

<p>
	(However, a different algorithm is used if
<code>gfm_auto_identifiers</code> is enabled; see below.)
	</p>
...
<p>
	If the <code>--section-divs</code> option is specified, then each section will
...
<p>
	Causes the identifiers produced by <code>auto_identifiers</code> to be pure ASCII.
...
<p>
	Changes the algorithm used by <code>auto_identifiers</code> to conform to
...
<p>
	The extensions <a href='#extension-tex_math_dollars' title=''><code>tex_math_dollars</code></a>,
...
sections of <a href='#pandocs-markdown' title=''>Pandoc’s Markdown</a>.
	</p>
...
<p>
	This extension can be enabled/disabled for the following formats
(in addition to <code>markdown</code>):
	</p>
...
<p>
	Note: as applied to <code>ipynb</code>, <code>raw_html</code> and <code>raw_tex</code> affect not
...
allow raw <code>html</code> or <code>tex</code>.
	</p>
...
can parse them to raw HTML using <code>-f html-native_divs+raw_html</code>.)
	</p>

<p>
	When converting HTML to Markdown, for example, you may want to drop all
<code>div</code>s and <code>span</code>s:
	</p>
...
<p>
	Analogous to <code>native_divs</code> above.
	</p>
...
<p>
	If you append <code>+lhs</code> (or <code>+literate_haskell</code>) to one of the formats
...
<li><p>
	In Markdown output, code blocks with classes <code>haskell</code> and <code>literate</code>
...
<li><p>
	In restructured text output, code blocks with class <code>haskell</code> will
be rendered using bird tracks.
	</p></li>
<li><p>
	In LaTeX input, text in <code>code</code> environments will be parsed as
Haskell code.
	</p></li>
<li><p>
	In LaTeX output, code blocks with class <code>haskell</code> will be rendered
inside <code>code</code> environments.
	</p></li>
<li><p>
	In HTML output, code blocks with class <code>haskell</code> will be rendered
with class <code>literatehaskell</code> and bird tracks.
	</p></li>
</ul>
...
<p>
	In the <code>muse</code> input format, this enables Text::Amuse
...
<p>
	Some aspects of <a href='#citations' title=''>Pandoc’s Markdown citation syntax</a> are also accepted
in <code>org</code> input.
	</p>

<h4 id="extension-ntb">Extension: <code>ntb</code></h4>

<p>
	In the <code>context</code> output format this enables the use of <a href='http://wiki.contextgarden.net/TABLE' title=''>Natural Tables
...
<a href='#extensions' title=''>Extensions</a> above, for extensions that work also on other formats.
	</p>
...
instructions.
– <a href='http://daringfireball.net/projects/markdown/syntax#philosophy' title=''>John Gruber</a>
	</p>
</blockquote>
...
<p>
	A setext-style header is a line of text <q>underlined</q> with a row of <code>=</code> signs
(for a level one header) or <code>-</code> signs (for a level two header):
	</p>
...
<a href='#inline-formatting' title=''>Inline formatting</a>, below).
	</p>

<h3 id="atx-style-headers">ATX-style headers</h3>

<p>
	An ATX-style header consists of one to six <code>#</code> signs and a line of
text, optionally followed by any number of <code>#</code> signs. The number of
<code>#</code> signs at the beginning of the line is the header level:
	</p>
...
<p>
	See also the <a href='#extension-auto_identifiers' title=''><code>auto_identifiers</code> extension</a> above.
	</p>
...
<p>
	(This syntax is compatible with <a href='https://michelf.ca/projects/php-markdown/extra/' title=''>PHP Markdown Extra</a>.)
	</p>
...
<p>
	Headers with the class <code>unnumbered</code> will not be numbered, even if
...
link will point to <code>bar</code>, not to <code>#foo</code>:
	</p>
...
<p>
	A <q>lazy</q> form, which requires the <code>&gt;</code> character only on the first
...
<p>
	If the <code>&gt;</code> character is followed by an optional space, that space
...
block in a block quote, you need five spaces after the <code>&gt;</code>:
	</p>
...
<p>
	Same as <code>fenced_code_blocks</code>, but uses backticks (<code>`</code>) instead of tildes
(<code>~</code>).
	</p>
...
<p>
	Here <code>mycode</code> is an identifier, <code>haskell</code> and <code>numberLines</code> are classes, and
...
<p>
	The <code>numberLines</code> (or <code>number-lines</code>) class will cause the lines
...
<p>
	If the <code>fenced_code_attributes</code> extension is disabled, but
...
<p>
	To prevent all highlighting, use the <code>--no-highlight</code> flag.
...
<p>
	A line block is a sequence of lines beginning with a vertical bar (<code>|</code>)
...
<p>
	This syntax is borrowed from <a href='http://docutils.sourceforge.net/docs/ref/rst/introduction.html' title=''>reStructuredText</a>.
	</p>
...
<p>
	The <code>fancy_lists</code> extension also allows <sq><code>#</code></sq> to be used as an
...
<a href='https://michelf.ca/projects/php-markdown/extra/' title=''>PHP Markdown Extra</a> with some extensions.<a id="fnref2" href="#fn2"><sup>2</sup></a>
	</p>
...
<a href='#non-pandoc-extensions' title=''>Non-pandoc extensions</a>, below.)
	</p>
...
<p>
	The special list marker <code>@</code> can be used for sequentially numbered
examples. The first list item with a <code>@</code> marker will be numbered <sq>1</sq>,
...
<p>
	Pandoc behaves differently from <code>Markdown.pl</code> on some <q>edge
...
<p>
	Pandoc transforms this into a <q>compact list</q> (with no <code>&lt;p&gt;</code> tags around
<q>First</q>, <q>Second</q>, or <q>Third</q>), while Markdown puts <code>&lt;p&gt;</code> tags around
...
even though it is different from that of <code>Markdown.pl</code>.)
	</p>
...
<p>
	A line containing a row of three or more <code>*</code>, <code>-</code>, or <code>_</code> characters
...
<p>
	The row of <code>=</code>s separates the header from the table body, and can be
omitted for a headerless table. The cells of grid tables may contain
arbitrary block elements (multiple paragraphs, code blocks, lists,
etc.). Cells that span multiple columns or rows are not
supported. Grid tables can be created easily using <a href='http://table.sourceforge.net/' title=''>Emacs table mode</a>.
	</p>
...
<p>
	The syntax is identical to <a href='https://michelf.ca/projects/php-markdown/extra/#table' title=''>PHP Markdown Extra tables</a>. The beginning and
...
<p>
	The difference is that <code>+</code> is used instead of <code>|</code>. Other orgtbl features
...
<p>
	Just be sure that the YAML file begins with <code>---</code> and ends with <code>---</code> or
...
example, <code>yes</code>, <code>True</code>, and <code>15</code> cannot be used as field names).
	</p>
...
<p>
	When pandoc is used with <code>-t markdown</code> to create a Markdown document,
...
equivalent of the Markdown in the <code>abstract</code> field:
	</p>
...
<p>
	(However, if the <code>markdown_strict</code> format is used, the standard Markdown rule
will be used.)
	</p>
...
<p>
	To <em>emphasize</em> some text, surround it with <code>*</code>s or <code>_</code>, like this:
	</p>
...
<p>
	Double <code>*</code> or <code>_</code> produces <alert>strong emphasis</alert>:
	</p>

<pre><code>This is **strong emphasis** and __with underscores__.</code></pre>

<p>
	A <code>*</code> or <code>_</code> character surrounded by spaces, or backslash-escaped,
...
<p>
	Because <code>_</code> is sometimes used inside words and identifiers,
...
just part of a word, use <code>*</code>:
	</p>
...
with <code>~~</code>. Thus, for example,
	</p>
...
<p>
	Superscripts may be written by surrounding the superscripted text by <code>^</code>
characters; subscripts may be written by surrounding the subscripted
text by <code>~</code> characters. Thus, for example,
	</p>
...
<code>P~a\ cat~</code>, not <code>P~a cat~</code>.
	</p>
...
<a href='#fenced-code-blocks' title=''>fenced code blocks</a>:
	</p>
...
<p>
	To write small caps, use the <code>smallcaps</code> class:
	</p>

<pre><code>[Small caps]{.smallcaps}</code></pre>

<p>
	Or, without the <code>bracketed_spans</code> extension:
	</p>
...
<p>
	Anything between two <code>$</code> characters will be treated as TeX math. The
...
document, see the <a href='#extension-raw_attribute' title=''><code>raw_attribute</code> extension</a>.
	</p>

<p>
	In the CommonMark format, if <code>raw_html</code> is enabled, superscripts,
...
so (for example), <code>*</code> does not signify emphasis.
	</p>

<p>
	Pandoc behaves this way when the <code>markdown_strict</code> format is used; but
...
<p>
	There is one exception to this rule: text between <code>&lt;script&gt;</code> and
<code>&lt;style&gt;</code> tags is not interpreted as Markdown.
	</p>
...
<p>
	Use native pandoc <code>Div</code> blocks for content inside <code>&lt;div&gt;</code> tags.
...
<p>
	Use native pandoc <code>Span</code> blocks for content inside <code>&lt;span&gt;</code> tags.
...
<p>
	And the following produces a raw <code>html</code> inline element:
	</p>

<pre><code>This is `&lt;a&gt;html&lt;/a&gt;`{=html}</code></pre>

<p>
	This can be useful to insert raw xml into <code>docx</code> documents, e.g.
a pagebreak:
	</p>
...
<code>backtick_code_blocks</code> must be enabled.
	</p>
...
<a href='#extension-raw_attribute' title=''><code>raw_attribute</code> extension</a>.
	</p>

<p>
	When <code>latex_macros</code> is disabled, the raw LaTeX and math will
not have macros applied. This is usually a better approach when
you are targeting LaTeX or PDF.
	</p>

<p>
	Whether or not <code>latex_macros</code> is enabled, the macro definitions
...
prefixed with <code>mailto</code>:
	</p>
...
<p>
	Note: In <code>Markdown.pl</code> and most other Markdown implementations,
...
generated identifier (see <a href='#header-identifiers' title=''>Header identifiers</a>). For example:
	</p>
...
<p>
	A link immediately preceded by a <code>!</code> will be treated as an image.
...
<p>
	(This syntax is compatible with <a href='https://michelf.ca/projects/php-markdown/extra/' title=''>PHP Markdown Extra</a> when only <code>#id</code>
and <code>.class</code> are used.)
	</p>

<p>
	For HTML and EPUB, all attributes except <code>width</code> and <code>height</code> (but
...
<p>
	The <code>width</code> and <code>height</code> attributes on images are treated specially. When
...
<p>
	Using the <code>native_divs</code> and <code>native_spans</code> extensions
...
<p>
	Allow special fenced syntax for native <code>Div</code> blocks. A Div
...
<p>
	Using an external filter, <code>pandoc-citeproc</code>, pandoc can automatically generate
...
<p>
	Note that <code>.bib</code> can be used with both BibTeX and BibLaTeX files;
use <code>.bibtex</code> to force BibTeX.
	</p>

<p>
	Note that <code>pandoc-citeproc --bib2json</code> and <code>pandoc-citeproc --bib2yaml</code>
can produce <code>.json</code> and <code>.yaml</code> files from any of the supported formats.
	</p>
...
<a href='http://docs.citationstyles.org/en/1.0/release-notes.html#rich-text-markup-within-fields' title=''>HTML-like markup</a>:
	</p>
...
<p>
	<code>pandoc-citeproc -j</code> and <code>-y</code> interconvert the CSL JSON
and CSL YAML formats as far as possible.
	</p>

<p>
	As an alternative to specifying a bibliography file using <code>--bibliography</code>
...
<p>
	(<code>pandoc-citeproc --bib2yaml</code> can produce these from a bibliography file in one
...
format. The CSL project provides further information on <a href='https://citationstyles.org/authors/' title=''>finding and editing styles</a>.
	</p>

<p>
	To make your citations hyperlinks to the corresponding bibliography
entries, add <code>link-citations: true</code> to your YAML metadata.
	</p>
...
characters (<code>:.#$%&amp;-+?&lt;&gt;~/</code>). Here are some examples:
	</p>
...
<p>
	<code>pandoc-citeproc</code> detects locator terms in the <a href='https://github.com/citation-style-language/locales' title=''>CSL locale files</a>.
...
<p>
	<code>pandoc-citeproc</code> will use heuristics to distinguish the locator
from the suffix. In complex cases, the locator can be enclosed
in curly braces (using <code>pandoc-citeproc</code> 0.15 and higher only):
	</p>
...
<p>
	A minus sign (<code>-</code>) before the <code>@</code> will suppress mention of
...
in a div with id <code>refs</code>, if one exists:
	</p>
...
<code>suppress-bibliography: true</code> in the YAML metadata.
	</p>
...
<p>
	In this example, the document will contain a citation for <code>item3</code>
...
<p>
	For LaTeX output, you can also use <a href='https://ctan.org/pkg/natbib' title=''><code>natbib</code></a> or <a href='https://ctan.org/pkg/biblatex' title=''><code>biblatex</code></a> to
...
<p>
	For more information, see the <a href='https://github.com/jgm/pandoc-citeproc/blob/master/man/pandoc-citeproc.1.md' title=''>pandoc-citeproc man page</a>.
	</p>
...
example, <code>markdown+hard_line_breaks</code> is Markdown with hard line breaks.
	</p>
...
selected automatically for <code>textile</code> input.
	</p>

<h4 id="extension-angle_brackets_escapable">Extension: <code>angle_brackets_escapable</code></h4>

<p>
	Allow <code>&lt;</code> and <code>&gt;</code> to be backslash-escaped, as they can be in
GitHub flavored Markdown but not original Markdown. This is
implied by pandoc’s default <code>all_symbols_escapable</code>.
	</p>
...
<p>
	Parses textual emojis like <code>:smile:</code> as Unicode emoticons.
	</p>

<h4 id="extension-tex_math_single_backslash">Extension: <code>tex_math_single_backslash</code></h4>

<p>
	Causes anything between <code>\(</code> and <code>\)</code> to be interpreted as inline
...
it precludes escaping <code>(</code> and <code>[</code>.
	</p>
...
inside block-level tags if the tags have the attribute <code>markdown=1</code>.
	</p>

<h4 id="extension-mmd_title_block">Extension: <code>mmd_title_block</code></h4>

<p>
	Enables a <a href='http://fletcherpenney.net/multimarkdown/' title=''>MultiMarkdown</a> style title block at the top of
...
<p>
	See the MultiMarkdown documentation for details. If <code>pandoc_title_block</code> or
<code>yaml_metadata_block</code> is enabled, it will take precedence over
<code>mmd_title_block</code>.
	</p>
...
<a href='#extension-link_attributes' title=''><code>link_attributes</code></a> extension.
	</p>
...
after the header but before any trailing <code>#</code>s in an ATX header).
	</p>
...
<p>
	We also support <code>commonmark</code> and <code>gfm</code> (GitHub-Flavored Markdown,
which is implemented as a set of extensions on <code>commonmark</code>).
	</p>

<p>
	Note, however, that <code>commonmark</code> and <code>gfm</code> have limited support
...
only affects <code>gfm</code> output, not input.
	</p>
...
slides shows in Microsoft <a href='https://en.wikipedia.org/wiki/Microsoft_PowerPoint' title=''>PowerPoint</a> format.
	</p>
...
<p>
	where <code>FORMAT</code> is either <code>s5</code>, <code>slidy</code>, <code>slideous</code>, <code>dzslides</code>, or <code>revealjs</code>.
	</p>
...
<p>
	With all HTML slide formats, the <code>--self-contained</code> option can be used to
...
the <code>--slide-level</code> option.
	</p>
...
<p>
	While using <code>incremental</code> and <code>nonincremental</code> divs are the
...
<p>
	All <a href='https://github.com/hakimel/reveal.js#configuration' title=''>reveal.js configuration options</a> can be set through variables.
For example, themes can be used by setting the <code>theme</code> variable:
	</p>

<pre><code>-V theme=moon</code></pre>

<p>
	Or you can specify a custom stylesheet using the <code>--css</code> option.
	</p>

<p>
	To style beamer slides, you can specify a <code>theme</code>, <code>colortheme</code>,
<code>fonttheme</code>, <code>innertheme</code>, and <code>outertheme</code>, using the <code>-V</code> option:
	</p>
...
<p>
	To show the notes window in reveal.js, press <code>s</code> while viewing the
...
containers with class <code>column</code> and a <code>width</code> attribute:
	</p>
...
<p>
	Sometimes it is necessary to add the LaTeX <code>[fragile]</code> option to
...
<code>shrink</code>, <code>standout</code>, <code>noframenumbering</code>.
	</p>
...
<code>parallaxBackgroundSize</code> to have your values take effect.
	</p>
...
<code>data-transition</code>, and <code>data-transition-speed</code>.
	</p>

<p>
	See the <a href='https://github.com/hakimel/reveal.js#slide-backgrounds' title=''>reveal.js
documentation</a>
for more details.
	</p>
...
<p>
	For <code>epub3</code> output, you can mark up the header that corresponds to an EPUB
...
the attribute to the value <code>prologue</code>, use this markdown:
	</p>
...
<p>
	Pandoc will output <code>&lt;body epub:type=&quot;bodymatter&quot;&gt;</code>, unless
you use one of the following values, in which case either
<code>frontmatter</code> or <code>backmatter</code> will be output.
	</p>
...
<p>
	By default, pandoc will download media referenced from any <code>&lt;img&gt;</code>, <code>&lt;audio&gt;</code>,
...
with the <code>src</code> attribute. For example:
	</p>
...
<p>
	When creating a <a href='https://nbformat.readthedocs.io/en/latest/' title=''>Jupyter notebook</a>, pandoc will try to infer the
...
<code>jupyter</code> metadata field. For example:
	</p>
...
divs</a> or <a href='#extension-native_divs' title=''>native divs</a> for this. Here is an example:
	</p>
...
<p>
	Pandoc will automatically highlight syntax in <a href='#fenced-code-blocks' title=''>fenced code blocks</a> that
...
<p>
	The color scheme can be selected using the <code>--highlight-style</code> option.
...
type <code>pandoc --list-highlight-styles</code>.
	</p>
...
get a JSON version of the <code>pygments</code> style, for example:
	</p>

<pre><code>pandoc --print-highlight-style pygments &gt; my.theme</code></pre>

<p>
	Then edit <code>my.theme</code> and use it like this:
	</p>
...
<p>
	To disable highlighting, use the <code>--no-highlight</code> option.
	</p>
...
using <code>div</code>s and <code>span</code>s, respectively.
	</p>

<p>
	If you define a <code>div</code> or <code>span</code> with the attribute <code>custom-style</code>,
pandoc will apply your specified style to the contained elements. So,
for example using the <code>bracketed_spans</code> syntax,
	</p>
...
style <code>Emphatically</code>. Similarly, using the <code>fenced_divs</code> syntax,
	</p>
...
<p>
	would style the two contained lines with the <code>Poetry</code> paragraph style.
	</p>
...
<p>
	By enabling the <a href='#ext-styles' title=''><code>styles</code> extension</a> in the docx reader
...
<p>
	For example, using the <code>custom-style-reference.docx</code> file in the test
directory, we have the following different outputs:
	</p>

<p>
	Without the <code>+styles</code> extension:
	</p>
...
<p>
	Pandoc can be extended with custom writers written in <a href='http://www.lua.org' title=''>lua</a>. (Pandoc
...
<li id="fn2"><p>
	I have been influenced by the suggestions of <a href='http://www.justatheory.com/computers/markup/modest-markdown-proposal.html' title=''>David Wheeler</a>.
	 <a href="#fnref2">&#8617;</a></p></li>
<li id="fn3"><p>
	This scheme is due to Michel Fortin, who proposed it on the
<a href='http://six.pairlist.net/pipermail/markdown-discuss/2005-March/001097.html' title=''>Markdown discussion list</a>.
